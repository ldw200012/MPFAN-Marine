...
class Batched_ED_Plus(Function):
    ...
    @staticmethod
    def forward(ctx, input):
        # Assuming input is of shape (B, N, 3, 3)
        B, N, dim1, dim2 = input.shape
        assert dim1 == 3 and dim2 == 3, "Each matrix must be 3x3."
        input_dim = dim1

        # Prepare to collect eigenvectors and eigenvalues
        eig_vecs = torch.empty(B, N, input_dim, input_dim, dtype=input.dtype, device=input.device)
        eig_vals = torch.empty(B, N, input_dim, dtype=input.dtype, device=input.device)

        for b in range(B):
            cov_matrix = input[b]
            tri_cov, tri_vector = Householder_reflection(cov_matrix, N, input_dim)  # Assume these functions handle single (3,3) matrices
            ...
            beta_list = []
            for i in range(break_times-1):
                tri_smaller, covnorm, cov_beta = DivideStep(tri_cov)  # Adjust these functions to handle (3,3) matrices
                norm_list.append(covnorm)
                beta_list.append(cov_beta)
                tri_cov = tri_smaller
            ...
            # QR iteration to compute smallest block
            conquer_index = break_times-2
            fused_diag, fused_vector = QR_Givens_Shift(tri_cov,tri_cov.size(0),tri_cov.size(1),tri_cov.size(1))
            ...
            eig_diag = torch.diag_embed(torch.clamp(torch.diagonal(fused_diag, dim1=-2, dim2=-1), min=1e-10))
            eig_vec = tri_vector.bmm(fused_vector)

            # Store the results for this point
            eig_vecs[b] = eig_vec
            eig_vals[b] = torch.diagonal(eig_diag, dim1=-2, dim2=-1)

            ctx.save_for_backward(eig_vecs, eig_vals)
